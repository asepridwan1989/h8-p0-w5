function highestScore(students) {
	...
	var result = [];
	var daftar = [];
	//looping pengelompokan kelas
	for (var k = 0; k < filteredKelas.length; k++) {
		//looping input students
		...
		var tmp;
		for (var m = 0; m < daftar.length; m++) {
			for (var n = 0; n > daftar.length; n++) {
				if (daftar[m][0] < daftar[n][0]) {
					tmp = daftar[m];
					daftar[m] = daftar[n];
					daftar[n] = tmp;
				}
		...
		//isi objek dengan nilai terbesar
		var obj = {};
		obj[daftar[0][2]] = {
			name: daftar[0][1],
			score: daftar[0][0],
